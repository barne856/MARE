...
namespace mare
{
    ...
    public:
        ...
        {
            depth_buffer = Renderer::gen_framebuffer(oversample_ * Renderer::get_info().window_width, oversample_ * Renderer::get_info().window_height);
        ...
        void render(float dt, Camera *camera, Scene *scene) override
        {
            ...
            glViewport(0, 0, oversample_ * Renderer::get_info().window_width, oversample_ * Renderer::get_info().window_height);
            glEnable(GL_POLYGON_OFFSET_FILL);
            glPolygonOffset(4.0f, 4.0f);
            // Get entities with a shadow component
            auto shadable_entities = scene->get_entities<Shadow>();
            // render all meshes from the perspective of the light with a basic material to record depth into the depth buffer
            for(auto ent : shadable_entities)
            {
                for(auto pack_it = ent->packets_begin(); pack_it != ent->packets_end(); pack_it++)
                {
                    auto mesh = (*pack_it).first;
                    mesh->render(std::dynamic_pointer_cast<Camera>(spotlight).get(), material.get(), ent->get_model());
                }
            ...
            glViewport(0, 0, Renderer::get_info().window_width, Renderer::get_info().window_height);
