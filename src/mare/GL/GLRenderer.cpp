...
namespace mare
{
    ...
    void GLRenderer::set_framebuffer(Framebuffer *framebuffer)
    {
        if (framebuffer)
        {
    ...
    void GLRenderer::render_simple_mesh(Camera *camera, SimpleMesh *mesh, Material *material)
    {
        material->bind();
        mesh->bind(material);
    ...
    // composite rendering
    void GLRenderer::render_simple_mesh(Camera *camera, SimpleMesh *mesh, Material *material, glm::mat4 &parent_model)
    {
        material->bind();
        mesh->bind(material);
        material->upload_camera(camera);
    ...
    // instanced rendering
    void GLRenderer::render_simple_mesh(Camera *camera, SimpleMesh *mesh, Material *material, glm::mat4 &parent_model, unsigned int instance_count, Buffer<glm::mat4> *models)
    {
        material->bind();
        mesh->bind(material);
        material->upload_camera(camera);
        ...
        else if (mesh->render_states.find(material->name()) == mesh->render_states.end())
        {
            ...
            for (auto &buffer : mesh->geometry_buffers)
            {
                for (const auto &attrib : buffer->format())
                {
                    ...
                    {
                        GLint attrib_loc = glGetAttribLocation(material->name(), attrib.name.c_str());
                        if (attrib_loc != -1)
                        {
