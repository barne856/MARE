...
namespace mare
{
    ...
    void GLShader::upload_image2D(const char *name, Texture2DBuffer *texture2D, bool suppress_warnings)
    {
        if (!resource_cache_.count(name))
        {
            // cache the location
            resource_cache_.insert_or_assign(name, glGetUniformLocation(shader_ID_, name));
            image_binding_cache_.insert_or_assign(name, static_cast<GLuint>(image_binding_cache_.size()));
            glUniform1i(resource_cache_[name], image_binding_cache_[name]);
        }
        glBindImageTexture(image_binding_cache_[name], texture2D->name(), 0, GL_FALSE, 0, GL_READ_WRITE, gl_sized_tex_format(texture2D->type()));
        if (resource_cache_[name] == -1 && suppress_warnings == false)
        {
            std::cerr << "SHADER WARNING: No uniform image2D '" << name << "' exists in the shader" << std::endl;
        }
    }
