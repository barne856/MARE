...
class GLCharMesh : public GLMesh, public CharMesh
{
public:
    GLCharMesh(std::string str, float keying = 1.0f) : CharMesh(str, keying) //, bool dynamic, size_t size_in_bytes)
    {
        // Set defaults
        this->keying = keying;
    ...
    void set_center(glm::vec3 center) override
    {
        glm::vec2 label_center = {max_width / 2.0f, lines / 2.0f};
        glm::vec3 label_scale = get_scale();
...
private:
    ...
    std::vector<std::vector<float>> create_letter_list(std::string str)
    {
        ...
        for (int i = 0; i < str.length(); i++)
        {
            ...
            letter_list.push_back(move_down(move_right(char_to_lines(str[i]), float(caret_pos) * keying), lines));
            caret_pos += 1;
            line_width += 1;
